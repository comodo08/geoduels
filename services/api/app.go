...
func routes(a *api) *mux.Router {
	...
	r.HandleFunc("/v1/users/search", a.searchPlayers).Methods(http.MethodGet)
	r.HandleFunc("/v1/me/friends", a.listFriends).Methods(http.MethodGet)
	r.HandleFunc("/v1/me/friends/requests", a.listFriendRequests).Methods(http.MethodGet)
	r.HandleFunc("/v1/me/friends/requests", a.createFriendRequest).Methods(http.MethodPost)
	r.HandleFunc("/v1/me/friends/requests/{requesterId}/accept", a.acceptFriendRequest).Methods(http.MethodPost)
	r.HandleFunc("/v1/me/friends/requests/{requesterId}/decline", a.declineFriendRequest).Methods(http.MethodPost)
	r.HandleFunc("/v1/me/friends/{friendUserId}", a.removeFriend).Methods(http.MethodDelete)
	r.HandleFunc("/v1/parties/{partyId}/invite-friend", a.inviteFriendToParty).Methods(http.MethodPost)
	r.HandleFunc("/v1/social/ws", a.socialWS)
