...
// The Go persistence layer embeds its own copy of the catalog; both outputs are
// committed so backend and web Docker builds stay independent.
const backendCatalogPath = path.join(root, "..", "backend", "pkg", "mapthumbnails", "catalog.generated.json");
const catalogPaths = [catalogPath, backendCatalogPath];
  ...
  if (checkOnly) {
    for (const catalogPath of catalogPaths) {
      ...
      try {
        ...
        console.error(`Missing thumbnail catalog copy: ${catalogPath}`);
        process.exitCode = 1;
        return;
      }
      if (existingCatalog !== content) {
        console.error(`Thumbnail picker catalog is out of date: ${catalogPath}. Run npm run maps:thumbnails.`);
        process.exitCode = 1;
        return;
      }
    }
