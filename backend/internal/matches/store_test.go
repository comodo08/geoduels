...
func TestRiskEngineSignalsDoNotCreateLegacyCases(t *testing.T) {
	body, err := os.ReadFile("../moderation/moderation_enforcement.go")
	if err != nil {
		t.Fatal(err)
	}
	source := string(body)
...
func TestNullableTeamEnumsAreCastBeforeEmptyStringFallback(t *testing.T) {
	for _, name := range []string{"../parties/parties.go", "../chat/chat.go"} {
...
func TestEnteringTeamDuelShufflesMembersIntoBalancedTeams(t *testing.T) {
	goBody, err := os.ReadFile("../parties/parties.go")
	if err != nil {
		t.Fatal(err)
	}
