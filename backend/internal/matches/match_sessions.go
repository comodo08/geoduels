...
func anySessionText(v any, fallback string) string {
	if value := storekit.TextVal(v); value != "" {
		return value
	}
	return fallback
}
...
func (s *PGStore) MatchSessionSourceParty(ctx context.Context, id string) (string, string, bool, error) {
	...
	sourcePartyID := storekit.UUIDVal(source.SourcePartyID)
	return sourcePartyID, source.SourcePartyInviteCode, sourcePartyID != "", nil
}
func (s *PGStore) MatchSessionReturnTarget(ctx context.Context, id string) (*contracts.MatchReturnTarget, bool, error) {
	id = strings.TrimSpace(id)
	if id == "" {
...
func (s *PGStore) RenewMatchSessionLeases(node string, epoch int64, ids []string, ttl time.Duration) error {
	...
	return s.db.RenewMatchSessionLeases(ctx, db.RenewMatchSessionLeasesParams{
		...
		MatchIds:  storekit.UUIDList(ids),
