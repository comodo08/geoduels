...
func TestFinalizeDuelDrawIntegration(t *testing.T) {
	...
	defer pool.Close()
	store := &PGStore{pool: pool, db: db.New(pool)}
...
func TestStaleMatchLeaseReconciliationIntegrationMovedToStorage(t *testing.T) {
	t.Skip("reconciliation lives on internal/storage")
}
