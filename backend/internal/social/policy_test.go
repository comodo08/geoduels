...
func TestAuthorizeUsesSharedSocialRules(t *testing.T) {
	tests := []struct {
		...
		{name: "blocked", in: Account{Blocked: true, ActionEnabled: true, TargetExists: true}, want: ErrBlocked},
		{name: "disabled", in: Account{TargetExists: true}, want: ErrBlocked},
		{name: "limit", in: Account{ActionEnabled: true, TargetExists: true, AtLimit: true}, want: ErrLimit},
	}
