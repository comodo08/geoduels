...
func (s *PGStore) CreatePlayerReportSignal(p CreatePlayerReportSignalParams) (ModerationSignalCreated, error) {
	...
	np, _ := json.Marshal(ModerationSignalNotificationPayload{SignalID: id, SubjectUserID: storekit.UUIDVal(r.SubjectUserID), SubjectName: storekit.TextVal(r.SubjectDisplayName), Severity: string(r.Severity), EvidenceStrength: string(r.EvidenceStrength), ReasonCode: string(r.ReasonCode), OccurredAt: r.OccurredAt.Time})
...
func (s *PGStore) ListSubjectModerationProfile(id string) (ModerationSubjectProfile, error) {
	...
	defer c()
	p, e := s.players.GetPlayerSummary(ctx, id)
	if e != nil {
		return ModerationSubjectProfile{}, e
	}
	st, e := s.players.GetPlayerStats(ctx, id)
	if e != nil {
		return ModerationSubjectProfile{}, e
	}
	u, e := profileUUID(id)
	...
	admin.ApplyPlayerStats(&p, st)
	return ModerationSubjectProfile{Player: p, Signals: sg, Log: lg}, nil
}
func (s *PGStore) ListModerationSignals(n int) ([]ModerationSignalSummary, error) {
	if n <= 0 {
		n = 100
	}
	if n > 200 {
...
func (s *PGStore) ListModerationLog(n int) ([]ModerationAuditLogEntry, error) {
	if n <= 0 {
		n = 100
	}
	if n > 200 {
...
func (s *PGStore) listSignals(ctx context.Context, u pgtype.UUID, n int32) ([]ModerationSignalSummary, error) {
	...
	for _, x := range r {
		item := ModerationSignalSummary{ID: x.SignalID, SubjectUserID: storekit.UUIDVal(x.SubjectUserID), SubjectName: storekit.TextVal(x.SubjectDisplayName), SignalType: x.SignalType, Source: string(x.Source), Severity: string(x.Severity), EvidenceStrength: string(x.EvidenceStrength), DetectorKey: x.DetectorKey, DetectorVersion: x.DetectorVersion, ReasonCode: x.ReasonCode, Score: x.Score, RecommendedQueue: x.RecommendedQueue, ReporterUserID: storekit.UUIDVal(x.ReporterUserID), ReporterName: storekit.TextVal(x.ReporterDisplayName), MatchID: storekit.UUIDVal(x.MatchID), Payload: json.RawMessage(x.PayloadJson), OccurredAt: x.OccurredAt.Time, CreatedAt: x.CreatedAt.Time, ReviewedAt: x.ReviewedAt.Time, ReviewedBy: storekit.UUIDVal(x.ReviewedBy)}
		if x.Outcome.Valid {
...
func (s *PGStore) listModerationLog(ctx context.Context, u pgtype.UUID, n int32) ([]ModerationAuditLogEntry, error) {
...
func enqueueSignalNotification(ctx context.Context, tx pgx.Tx, signalID int64) error {
	...
	p, _ := json.Marshal(ModerationSignalNotificationPayload{SignalID: signalID, SubjectUserID: storekit.UUIDVal(r.SubjectUserID), SubjectName: storekit.TextVal(r.SubjectDisplayName), Severity: string(r.Severity), EvidenceStrength: string(r.EvidenceStrength), ReasonCode: r.ReasonCode, OccurredAt: r.OccurredAt.Time})
