...
func TestBadgeFromPartsUsesStableLevelAndExtraState(t *testing.T) {
	legacy := FromParts(badgeCodeLegacyTopFinish, 1, 7, true)
	...
	for count, wantLevel := range map[int16]int16{1: 1, 2: 2, 3: 3, 4: 3, 99: 3} {
		...
		top := FromParts(badgeCodeTopFinish, got, count, true)
...
func TestBadgeRefRejectsRetiredLegacyAward(t *testing.T) {
	if code, ok := RefFromID("geoduels-v1-top-finish"); !ok || code != badgeCodeLegacyTopFinish {
		t.Fatalf("legacy badge ref = %d, %v", code, ok)
	}
	if code, ok := RefFromID("top-finish"); !ok || code != badgeCodeTopFinish {
