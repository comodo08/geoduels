...
func (s *PGStore) ReplaceMapLocations(mapKey, displayName string, dataset []byte) (contracts.MapImportSummary, error) {
	mapKey = strings.TrimSpace(mapKey)
	if mapKey == "" {
		return contracts.MapImportSummary{}, errors.New("map key required")
	}
	...
	if len(rows) == 0 {
		return contracts.MapImportSummary{}, errors.New("no valid rows")
	}
	...
	return contracts.MapImportSummary{MapID: mapID, MapKey: mapKey, LocationCount: len(rows), DisplayName: displayName}, nil
}
